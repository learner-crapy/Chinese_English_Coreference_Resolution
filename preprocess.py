...
# 通过这个类得到的是文件中的第k行到第m行的数据，共m-k个sample
class CRProcessor_BIG:

    # @staticmethod
    def read_json_k_lines(self, file_path, k, m):
        return self.read_k_lines(file_path, k, m)

    # 读取文件的第k行到第m行，不要将整个文件读入内存，尽量减小内存占用
    def read_k_lines(self, file_path, k, m):
        with open(file_path, encoding='utf-8') as f:
            for i, line in enumerate(f):
                if i >= k and i < m:
                    if line.strip() != '':
...
def convert_rc_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    span1_start = span1[1]
    span1_end = span1_start + len(span1[0].split(' '))

    for i in range(span1_start, span1_end):
        span1_ids[i] = 1
    span2_ids = [0] * len(tokens)
    span2_start = span2[1]
    span2_end = span2_start + len(span2[0].split(' '))
...
def get_data_big(processor, json_file, mode, args, k, m):
    raw_examples = processor.read_json_k_lines(os.path.join(args.data_dir, json_file), k, m)
    examples = processor.get_examples(raw_examples, mode)
    data = convert_examples_to_features(examples, args.max_seq_len, args.bert_dir)
    return data
