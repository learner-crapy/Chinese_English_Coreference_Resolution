...
class BertForCR:
    ...
    def train(self, train_loader, dev_loader=None):
        ...
        for epoch in range(1, self.args.train_epochs + 1):
            ...
            if f1 > best_f1:
                ...
                pt_dir = os.path.join(self.args.output_dir, path_time + experiment)
    ...
    def dev(self, dev_loader, num_confusion_matrix=0):
        ...
        with torch.no_grad():
            ...
            self.DrawFusionMatrix(classes=['0', '1'], confusion_matrix=confusion_matrix, Title=Title,
...
if __name__ == '__main__':
    ...
    experiment = '-vgg165-3-new-note-300-chinese-1d'
    ...
    # args.other_lr = 2e-5
    path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    ...
    # experiment = '-CRModel_2dense5-3-new-note-300-chinese-1d'
    # path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    # log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    #
    # if raw.IfFolderExists(log_dir_path):
    ...
    # experiment = 'TextCNN5-3-new-note-300-chinese-1d'
    # path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    # log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    #
    # if raw.IfFolderExists(log_dir_path):
    ...
    experiment = '-inception5-3-new-note-300-chinese-1d'
    path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    ...
    experiment = '-Lenet55-3-new-note-300-chinese-1d'
    path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    ...
    # model = LstmModel(input_size=1024 * 3, hidden_size=1024, num_layers=8, num_classes=2, args=args)  # chinese
    model = LstmModel(input_size=2304, hidden_size=1024, num_layers=8, num_classes=2, args=args) # english
    # model = LSTM(input_size=2304, hidden_size=1024, layer_size=8, output_size=2, args=args)
    experiment = '-LstmModel5-3-new-note-300-chinese-1d'
    path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    ...
    # experiment = '-CRModel5-3-new-note-300-chinese-1d'
    # path_time = time.strftime("%Y-%m-%d %H-%M-%S", time.localtime())
    # log_dir_path = os.path.join('./log_dir/', path_time + experiment)
    #
    # if raw.IfFolderExists(log_dir_path):
