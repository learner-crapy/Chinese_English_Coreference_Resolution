...
def main(language, jsonlines_path, example_dict, json_path, seq_max_len=300):
    ...
    for i in range(0, line_nums):
        ...
        for j in range(0, len(index_list)):
            # 长度太长的文本不进行生成
            if len(sentece_list) >= seq_max_len:
                continue
            print('processing file: ', jsonlines_path, '-------------------------------- line', i, '----------------------------------useful!')
            # define a list variable to store how many true lables
            ture_lable_list_j = []
            # define a list variable to store how many false lables
            false_lable_list_j = []
            ...
            false_lable_list_j = false_lable_list_j[0:len(ture_lable_list_j)]
            # 保证样本按照一个正样本一个负样本进行排列，尽量使得每个batch得数据都平衡
